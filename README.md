...
4. 本地监听脚本只接受指定私聊和白名单命令；“今晚不关”仅接受当天卡片，“立即关机”允许从历史卡片触发，然后调用固定系统命令。
...
| 项目 | 当前行为 |
| --- | --- |
| 开机推送 | Windows 登录后启动 bridge；检测到 `codex` profile 上线后立即私聊推送一次。同一次 Windows 开机以系统启动时间去重，bridge 重启或启动脚本重复执行不会重复发送。发送失败会重试，只有成功后才记录状态。 |
...
| 手动查询 | 卡片底部的“查询额度”按钮写入本地 `quota.refresh` 命令；常驻监听器收到后调用同一脚本重新查询并发送新卡，不启动模型会话。 |
...
开机推送复用 `~/.lark-channel/startup/start-codex-bridge.ps1`：脚本先等待本地代理并启动 bridge，优先读取 `supervisor/profile-online` 日志；若当前 bridge 版本未生成该 JSONL 事件，则以本次开机后稳定运行的 bridge 进程作为兼容信号。随后调用现有额度脚本，并仅在飞书返回发送成功后写入去重状态。状态保存在 profile 的 launcher 日志目录，不新增常驻进程，也不触发模型会话。
...
- 卡片底部放置小号“查询额度”按钮；`CodexQuotaCardActions` 在用户登录后以 `S4U + Limited` 后台启动，每 2 秒检查一次新点击并去重，不显示 PowerShell 窗口。
...
3. “立即关机”带二次确认，历史卡片中的按钮也可随时触发；确认后只把该按钮临时改为“正在关机…”，5 秒后恢复“立即关机”并执行关机，不额外发送文字回执。原始卡片 JSON 会随消息 ID 缓存在本地；恢复失败时，下次监听器启动会补恢复。
...
- `Codex-Lark-Shutdown-Reminder-0030`：以 `S4U + Limited` 后台触发三次提醒，不显示 PowerShell 窗口。
- `Codex-Lark-Shutdown-Card-Actions`：用户登录后以 `S4U + Limited` 后台持续运行，每 3 秒检查一次按钮事件；每天 `00:29` 的触发器用于异常退出后的补启动。任务不设运行时限，失败后最多重启 3 次。监听器只处理本次启动后的新点击，避免重放历史事件；监听期间不调用模型，也不显示 PowerShell 窗口。

按钮命令带创建日期，例如 `shutdown.skip.20260913` 和 `shutdown.now.20260913`。监听器校验目标私聊 ID 并去重事件；旧日期的 `skip` 会被忽略，旧日期的 `now` 仍可执行，方便从历史卡片远程关机。飞书只允许更新最近 14 天内发送的卡片，因此更早卡片仍能触发关机，但可能无法把原卡改成“正在关机”。
...
4. 创建额度提醒和关机提醒 PowerShell 脚本，并注册本章所列 Windows 计划任务；在 bridge 开机启动脚本中加入上线检测、发送成功校验和每次开机仅发送一次额度卡的逻辑。
5. 所有定时查询和消息发送必须绕过模型会话；额度查询和关机按钮均使用本地白名单 cmd，不使用 __bridge_cb。
