...
- 卡片底部放置小号“查询额度”按钮；`CodexQuotaCardActions` 在用户登录后启动，每 2 秒检查一次新点击并去重。
...
- `Codex-Lark-Shutdown-Card-Actions`：用户登录后持续运行，每 3 秒检查一次按钮事件；每天 `00:29` 的触发器用于异常退出后的补启动。任务不设运行时限，失败后最多重启 3 次。监听器只处理本次启动后的新点击，避免重放历史事件；监听期间不调用模型。
